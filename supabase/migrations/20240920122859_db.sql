...
create table
    public."Workshop_Subscription"
(
    created_at      timestamp with time zone not null default now(),
    workshop_id     bigint                   not null,
    subscription_id bigint                   not null,
    constraint Workshop_Subscription_pkey primary key (workshop_id, subscription_id),
    constraint Workshop_Subscription_workshop_id_fkey foreign key (workshop_id) references "Workshop" (id) on update cascade on delete cascade,
    constraint Workshop_Subscription_subscription_id_fkey foreign key (subscription_id) references "Subscription" (id) on update cascade on delete cascade
) tablespace pg_default;
